...
int SCRFD::detect(const cv::Mat &rgb, std::vector<FaceObject> &faceobjects, float prob_threshold,
                  float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
...
int SCRFD::findPose(std::vector<FaceObject> &faceobjects) {
    for (size_t j = 0;
         j < faceobjects.size(); j++) {  // Đổi tên biến lặp thành `j` cho duyệt faceobjects
        const FaceObject &obj = faceobjects[j];

        float LMx[5], LMy[5];
        for (int i = 0; i < 5; ++i) {
            LMx[i] = obj.landmark[i].x;
            LMy[i] = obj.landmark[i].y;
        }

        // Calculate differences between eye landmarks
        float dPx_eyes = std::max((LMx[1] - LMx[0]), 1.0f);
        float dPy_eyes = (LMy[1] - LMy[0]);
        double angle = std::atan(dPy_eyes / dPx_eyes); // angle for rotation based on slope
        ...
        float LMxr[5], LMyr[5];
        for (int i = 0; i < 5; ++i) {
            LMxr[i] = alpha * LMx[i] + beta * LMy[i] + (1 - alpha) * LMx[2] / 2 - beta * LMy[2] / 2;
            LMyr[i] =
                    -beta * LMx[i] + alpha * LMy[i] + beta * LMx[2] / 2 + (1 - alpha) * LMy[2] / 2;
        }
        ...
        // Relative rotation - 0 degree is frontal, 90 degrees is profile
        double Xfrontal = (dXtot != 0) ? (-90 + 90 / 0.5 * dXnose / dXtot) : 0;
        double Yfrontal = (dYtot != 0) ? (-90 + 90 / 0.5 * dYnose / dYtot) : 0;

        // Update FaceObject with roll, yaw, pitch
        faceobjects[j].roll = angle * 180 / M_PI;
        faceobjects[j].yaw = Xfrontal;
        faceobjects[j].pitch = Yfrontal;
    }
    return 0;
}

std::string determineFaceDirection(double roll, double yaw, double pitch) {
    std::string direction = "Face direction: ";

    // Yaw - Quay sang trái hoặc phải
    if (yaw > 30) {
    ...
    // Pitch - Cúi xuống hoặc ngửa lên
    if (pitch > 30) {
        direction += "Head up, ";
    } else if (pitch < -30) {
        direction += "Head down, ";
    } else {
        direction += "Head level, ";
    }

    // Roll - Nghiêng đầu sang trái hoặc phải
    if (roll > 30) {
...
//        std::string direction = determineFaceDirection(obj.roll, obj.yaw, obj.pitch);
//        char text[1000];
//        sprintf(text, "%s ",
//                direction.c_str());
